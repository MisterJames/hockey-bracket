<template>
    <svg :width="svgWidth" :height="svgHeight" xmlns="http://www.w3.org/2000/svg">
      ...
      <g v-for="(match, index) in bracket.matchups?.west?.[0] ?? []" :key="'w1-' + match.id">
        ...
        />
        <LineConnector
          v-if="bracket.matchups?.west?.[1]?.[Math.floor(index / 2)]"
      ...
      <g v-for="(match, index) in bracket.matchups?.west?.[1] ?? []" :key="'w2-' + match.id">
        <MatchupSvg
          :x="startX + roundSpacing"
          :y="index * matchSpacingY * 2 + matchSpacingY / 2"
          :width="boxWidth"
          :height="boxHeight"
          :match="match"
          :round="1"
        />
        <LineConnector
          v-if="bracket.matchups?.west?.[2]?.[Math.floor(index / 2)]"
      ...
      <g v-for="(match, index) in bracket.matchups?.west?.[2] ?? []" :key="'w3-' + match.id">
        <MatchupSvg
          :x="startX + roundSpacing * 2"
          :y="index * matchSpacingY * 4 + matchSpacingY * 1.5"
          :width="boxWidth"
          :height="boxHeight"
          :match="match"
          :round="2"
      ...
      <g v-if="bracket.matchups?.final">
        <MatchupSvg
          :x="startX + roundSpacing * 3"
          :y="svgHeight / 2 - boxHeight / 2"
          :width="boxWidth"
          :height="boxHeight"
          :match="bracket.matchups.final"
          :round="3"
        />
      </g>
