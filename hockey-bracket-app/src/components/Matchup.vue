<template>
    <div class="bg-white rounded shadow p-2 border w-32">
      <div v-for="(team, teamIndex) in [match?.teamA, match?.teamB]" :key="team?.id || teamIndex">
        <div
          v-if="team"
          class="flex items-center justify-between space-x-2 cursor-pointer py-1 px-1 transition-all"
          :class="[
            {
        ...
        >
          ...
          <span
            ...
            :class="{ 'animate-[winPulse_0.3s_ease-in-out]': winEffects[team.id] }"
          >
            {{ getWins(team.id) }}
          </span>
        </div>
        <div
          v-else
          class="text-gray-400 italic text-sm text-center py-1"
        >
